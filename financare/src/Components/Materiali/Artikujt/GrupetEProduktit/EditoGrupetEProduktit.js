...
function EditoGrupetEProduktit(props) {
    ...
    useEffect(() => {
        const shfaqNjesineMatese = async () => {
            try {
                const njesiaMatese = await axios.get(`https://localhost:7285/api/GrupiProduktit/shfaqGrupinEProduktitSipasIDs?id=${props.id}`, authentikimi);
    ...
    function handleSubmit() {
        axios.put(`https://localhost:7285/api/GrupiProduktit/perditesoGrupinEProduktit?id=${njesiaMatese.idGrupiProduktit}`, njesiaMatese, authentikimi)
            .then(x => {
            ...
            .catch(error => {
                ...
                props.setPershkrimiMesazhit("Ndodhi nje gabim gjate perditesimit te grupit te produktit!")
                props.perditesoTeDhenat();
                props.shfaqmesazhin();
            });
    }
    ...
    const handleKontrolli = () => {
        if (
            isNullOrEmpty(njesiaMatese.grupiIProduktit)
        ) {
            setFushatEZbrazura(true);
        } else {

            if (konfirmoNjesineMatese == false && njesiteMatese.filter((item) => item.grupiIProduktit === njesiaMatese.grupiIProduktit).length !== 0) {
            ...
            <Modal className="modalEditShto" show={true} onHide={() => props.largo()}>
                ...
                <Modal.Body>
                    <Form>
                        ...
                        <Form.Group className="mb-3" controlId="exampleForm.ControlInput1">
                            <Form.Label>Grupi i Produktit<span style={{ color: "red" }}>*</span></Form.Label>
                            <Form.Control
